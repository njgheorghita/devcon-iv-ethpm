...
if len(list(VYPER_FOLDER.glob("*.vy"))) == 0:
    raise Exception(
        "No vyper contracts found in {0}, please add a vyper contract with the .vy file extension.".format(
            VYPER_FOLDER
        )
    )
...
print("")
print(
    "Manifest: {0} written to {1}/{2}".format(
        manifest["package_name"], MANIFEST_FOLDER, manifest_filename
    )
)
print("")
print("💰⭐️💰⭐️💰⭐️💰⭐️")
